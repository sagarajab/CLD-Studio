...
// Separate memoized chart component to prevent re-renders during drag
const MemoizedChart = memo(({ chartData, nodes, colors, simulationState }) => {
  // Memoize line components to prevent recreation during drag
...
const ChartLegend = memo(({ nodes, colors }) => {
  return (
    <div style={{
      display: 'flex',
      flexWrap: 'wrap',
      ...
      {nodes.map((node, index) => {
        ...
        return (
          <div key={node.id} style={{
            ...
            <span style={{ color: '#333' }}>{nodeLabel}</span>
          </div>
        )
      })}
    </div>
...
function PlotsModal({ isOpen, onClose }) {
  ...
  // Memoize chart data to prevent recalculation during drag
  const chartData = useMemo(() => {
    ...
    // Implement data sampling for performance optimization
    // If we have more than 100 data points, sample every nth point
    const maxDataPoints = 100
    const stepInterval = currentStep > maxDataPoints ? Math.ceil(currentStep / maxDataPoints) : 1
    ...
    // Always include the last data point if it wasn't included in sampling
    if (stepInterval > 1 && currentStep % stepInterval !== 0) {
      const lastDataPoint = { step: currentStep }
      const lastValues = simulationState.valueHistory[currentStep]
      
      nodes.forEach((node, nodeIndex) => {
        const nodeLabel = node.data.label || `Node ${node.id}`
        const value = lastValues[nodeIndex]
        lastDataPoint[nodeLabel] = typeof value === 'number' ? value : 0
      })
      
      dataPoints.push(lastDataPoint)
    }
  ...
  const chartDimensions = useMemo(() => ({
    ...
    height: size.height - 200 // Account for header, padding, and legend space
  }), [size.width, size.height])

  // Generate colors for each node - memoized to prevent recalculation
  ...
  }, [isDragging, isResizing, dragStart, resizeStart, originalSize, originalPosition, resizeDirection])

  // Optimize mouse up handler with useCallback
  ...
  }, [isOpen, onClose, isResizing, isDragging, handleMouseMove, handleMouseUp])
    ...
    >
      ...
      <div style={{ 
        ...
        <div style={{ 
          ...
          {chartData.length > 0 ? (
            <>
              <ChartLegend nodes={nodes} colors={colors} />
